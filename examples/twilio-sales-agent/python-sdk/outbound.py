...
# Debug logging flag (default off)
...
async def outbound_call_twiml(
    ...
    # Create a Stream (Twilio uses 8k µ-law JSON media; no format attr)
    stream = Stream(url=f"wss://{request.headers.get('host')}/outbound-media-stream")
